...
<div class="container">
	...
	<div class="panel panel-default">
						...
						<tbody>
						  <c:forEach var="result1" items="${resultList1}" varStatus="status">
							<tr>
								...
								<td><c:out value="${result1.count}" /> </a></td>
								<td><c:out value="${result1.writer}" /> </a></td>
								<td><c:out value="${result1.indate}" /> </a></td>
							</tr>
						  </c:forEach>
						</tbody>
					</table>
		...
		<div class="panel-body col-sm-6">
				...
				<div onclick="moveFreeBoard();">자유게시판</div>
				<div class="table-responsive">
					<table class="table table-hover">
						<thead>
							<tr>
						...
						<tbody>
							<c:forEach var="result2" items="${resultList2}" varStatus="status">
								<tr>
									...
									<td><c:out value="${result2.count}" /> </a></td>
									<td><c:out value="${result2.writer}" /> </a></td>
									<td><c:out value="${result2.indate}" /> </a></td>
								</tr>
						  </c:forEach>
						</tbody>
